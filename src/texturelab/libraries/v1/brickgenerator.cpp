...
void BrickGeneratorNode::init()
{
    ...
    // brick size
    auto sizeProps = this->createGroup("Size");
    sizeProps->collapsed = false;
    sizeProps->add(
        this->addFloatProp("brickWidth", "Brick Width", 0.9, 0, 1, 0.01));
    sizeProps->add(
        this->addFloatProp("brickHeight", "Brick Height", 0.9, 0, 1, 0.01));
    ...
    auto source = R""""(
            ...
            // HEIGHT FUNCTIONS
            float
            calculateHeight(vec2 brickId)
        {
            // height
            float heightMin = prop_heightMin;
            float heightMax = prop_heightMax;
            float heightBalance =
                prop_heightBalance; // threshold that decides whether to use
                                    // height variance or not
            float heightVariance =
                prop_heightVariance; // multiplies the heightMax-heightMin range

            // check whether or not there should be a height range in the first
            // place
            float balRand = _rand(vec2(_seed) + brickId * vec2(0.01));

            // if balRand is less than heightBalance it means it qualifies for a
            // random height. This way if heightBalance is 0 then we only use
            // the min luminance
        ...
        // float slopeBalance;// threshold that determines whether or not to use
        // slope float slopeVariation;// decreases the range of the slope
